...
Now, you might be wandering _¿why I used the `PIPENV_IGNORE_VIRTUALENVS=1` declaration on the creation of the virtual environment?_.
...
We're finally ready to start developing.
...
If you look at that last line, you can see how the `SECRET_KEY` is available as an **environment variable**. Now we have to change `first_app/settings.py` to load that variable from the environment:
...
![](python-runserver-hello-error.png)
![](python-runserver-hello-success.png)
